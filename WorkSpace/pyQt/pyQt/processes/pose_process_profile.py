...
class ProcessProfiler:
    ...
    def build_snapshot(self, mode, ring_resources):
        elapsed = max(time.perf_counter() - self.start_time, 1e-9)
        fps = self.frame_count / elapsed
        pose_cpu_percent = max(0.0, (time.process_time() - self.cpu_start) / elapsed * 100.0)
        ...
        skipped = ring_resources["skipped_count"].value

        snapshot = {
            "mode": str(mode),
            "window_sec": elapsed,
            "pose_fps": fps,
            "pose_cpu_percent": pose_cpu_percent,
            "ring_written": int(written),
            "ring_read": int(read),
            "ring_pending": int(pending),
            "ring_skipped": int(skipped),
            "ring_overrun": int(overrun),
        }
        key_map = {
            "read": "read_ms",
            ...
            "feature": "feature_ms",
            "gru": "gru_update_ms",
            "gru_result": "gru_result_ms",
            "calibration": "calibration_ms",
            "process": "process_ms",
            "e2e": "e2e_ms",
        }
        for source_key, json_key in key_map.items():
            avg, p95, maximum = self._stats(self.samples[source_key])
            snapshot[f"{json_key}_avg"] = avg
            snapshot[f"{json_key}_p95"] = p95
            snapshot[f"{json_key}_max"] = maximum
        return snapshot

    def build_report(self, mode, ring_resources):
        snapshot = self.build_snapshot(mode, ring_resources)
        fps = snapshot["pose_fps"]
        written = snapshot["ring_written"]
        read = snapshot["ring_read"]
        pending = snapshot["ring_pending"]
        overrun = snapshot["ring_overrun"]
        ...
        lines = [
            "",
            "================= POSE PROFILE =================",
            f"Mode={mode:<12} FPS={fps:6.2f} Frames={self.frame_count} CPU={snapshot['pose_cpu_percent']:6.2f}%",
...
def run_pose_process(
    ...
    if performance_dir is not None and performance_session_id is not None:
        performance_logger = JsonPerformanceLogger(
            Path(performance_dir) / "pose_profile.json",
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            if profiler.ready():
                ...
                if performance_logger is not None and mode == MODE_MEASURING:
                    performance_logger.append(
                        profiler.build_snapshot(mode, ring_resources)
                    )
                profiler.reset()
