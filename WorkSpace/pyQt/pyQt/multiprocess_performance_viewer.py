...
def build_summary(run):
    ...
    pose = run["pose"]
    return {
        "camera_fps": _avg(main, "camera_fps"),
        "pose_fps": _avg(pose, "pose_fps"),
        "pose_e2e_ms": _avg(pose, "e2e_ms_avg"),
        "main_cpu_percent": _avg(main, "main_cpu_percent"),
        "pose_cpu_percent": _avg(pose, "pose_cpu_percent"),
        "shared_memory_write_ms": _avg(main, "shared_memory_write_ms_avg"),
        "queue_latency_ms": _avg(pose, "queue_latency_ms_avg"),
        "mediapipe_ms": _avg(pose, "mediapipe_ms_avg"),
        "pending_max": _max(pose, "ring_pending"),
        "skip_final": _last(pose, "ring_skipped"),
        "overrun_final": _last(pose, "ring_overrun"),
        "main_samples": len((main or {}).get("samples", [])),
        "pose_samples": len((pose or {}).get("samples", [])),
    }
