...
- **Mediapipe Holistic Fine Tuning**:
  - 미디어파이프 내부 모델을 파인튜닝 후 새로운 랜드마크 구성.

- **자세 분석 모델**:
  - 사용자 자세를 실시간 분석해서 자세 판단을 내려주는 모델 구현.
...
역할 분담
이름	담당 역할
조병현	Pose 모델 구현 및 튜닝, 자세 데이터 수집
이종현	Face 모델 구현 및 튜닝, 피로도 데이터 수집
신동민	프론트엔드 구현, 리포트 웹 개발, 데이터 수집
최은비	하드웨어 구현, 부저 및 진동 알림 기능, 데이터 수집
담당 기능 상세
모델 파트
...
본 프로젝트는 1차적으로 MVP 구현을 목표로 하며, 카메라 기반 자세 분석, 실시간 알림, 일일 리포트 기능을 중심으로 개발합니다.

추후에는 사용자별 자세 패턴 학습, 장기 통계 분석, 맞춤형 자세 개선 가이드 제공 기능으로 확장할 수 있습니다.
